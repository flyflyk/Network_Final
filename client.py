...
def create_client_dashboard(acked_seqs, start_time, is_finished=False, total_time=0):
    ...
    layout.split_row(
        Layout(
            Panel(
                grid, title="[magenta]Upload Status[/magenta]", border_style="magenta"
            ),
            ratio=2,
        ),
...
def run_client():
    ...
    with Live(
        create_client_dashboard(acked_seqs, start_time),
                ...
                if seq not in acked_seqs:
                    ...
                    try:
                        client_socket.sendto(
                            msg.encode("utf-8"), (PROXY_IP, PROXY_PORT)
                        )
        ...
        final_layout = create_client_dashboard(
            acked_seqs, start_time, is_finished=True, total_time=total_duration
        )
