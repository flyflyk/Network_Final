...
def create_dashboard(received_packets, status_log):
    ...
    info_content = f"[bold cyan]Server Status (SCRAMBLED MODE)[/bold cyan]\n" \
                   f"Port: {SERVER_PORT}\n\n" \
    ...
    layout = Layout()
    layout.split_row(
        Layout(Panel(grid_panel_content, title="[bold yellow]Image Reconstruction (Arrival Order)[/bold yellow]", border_style="blue"), ratio=2),
...
def run_server():
    ...
    with Live(create_dashboard(received_packets, current_log), refresh_per_second=FPS, screen=True) as live:
        ...
        # Verification phase
        current_log = "[bold red]Image Complete (Scrambled)![/bold red]\n" \
                      "Verifying Integrity..."
        live.update(create_dashboard(received_packets, current_log))
        ...
        final_msg = f"[bold red]✘ RECONSTRUCTION SCRAMBLED[/bold red]\n\n" \
                    f"Sorted: 0%\n" \
                    f"Loss: 0%\n\n" \
